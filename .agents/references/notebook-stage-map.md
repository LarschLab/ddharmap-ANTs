...
3. **Spatial preparation** (`[13] [14] [14a] [7] [9] [11] [15] [19] [19a] [21] [22d] [24a] [22e]`)
   ...
   - `[12]` builds/reuses oriented 2D functional references from original motion-corrected stacks while preserving legacy reference filenames and `plane_refs` labels. It explicitly excludes the first selected raw TIFF block when preprocessing metadata shows that block 1 remains in the motion-corrected movie; data already preprocessed from later blocks only (for example `blocks: [2, 3]`) are not trimmed again.
   - `[14]`/`[14a]` run before voxel inference `[8]`; `[14a]` saves a signed-16-bit-corrected 8-bit anatomy NRRD with 2P XY orientation mirrored by default through the same metadata-driven orientation convention used for functional references, anatomy Z flipped to match the bottom-to-top confocal registration convention, and Y/X resized to `750x750` at `02_reg/00_preprocessing/2p_anatomy/<fish_id>_anatomy_2P_GCaMP.nrrd`, writes spatial header metadata plus a `.nrrd.json` sidecar, rebinds `ANAT_STACK_PATH` for downstream anatomy consumers, and intentionally avoids duplicate TIFF image outputs.
   ...
   - The staged CLI separates functional registration into three ordered writers: `register-functional-to-anatomy` writes NCC/ANTs intensity registration and the composed transform, `transform-functional-rois-to-anatomy` applies that exact transform to Suite2p labels, and `make-functional-registration-qc` renders intensity, label-overlay, plane-row, and NCC-profile PNGs only after transformed labels exist.
