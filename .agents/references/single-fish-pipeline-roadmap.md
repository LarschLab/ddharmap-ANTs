...
- `audit-hcr-activity-replay` is a separate read-only replay audit for HCR-centric `[50]` identified-cell activity tables. It calls `matching.build_hcr_activity_tables` from staged `plane_refs_summary.json`, Suite2p, anatomy labels, and staged HCR final pairs, uses the ROI master only as response lookup through `matching.hcr_response_lookup_from_roi_master_df`, overlays selected accepted `ants_rigid_affine` transformlists when present, checks AntsPyx (`ants`) availability and transform-file coverage, scores selected-ANTs and persisted affine-CSV transform variants in memory, finalizes via `matching.finalize_hcr_activity_export_tables`, compares row/key counts to accepted HCR outputs when present, writes no staged HCR CSV outputs, and keeps promotion disabled.
- `assign-hcr-identity` is a hybrid writer stage. It now requires staged ROI/anatomy geometry under `match-roi-to-anatomy/registration/` plus staged HCR/anatomy artifacts under `register-hcr-to-anatomy/confocal/aligned/` before writing the identity/HCR bundle into `--pipeline-root/assign-hcr-identity/registration`. It recomputes `anatomy_identity_lookup.csv` from staged HCR final-pair CSVs using the accepted gene order, recomputes `functional_roi_activity_identity.csv` by attaching the lookup to staged ROI/anatomy geometry while preserving pass-through trace-quality/response fields, stages `hcr_activity_status.csv`, `hcr_activity_status_summary.csv`, `conf_to_func_pairs_raw.csv`, `conf_to_func_pairs.csv`, and `hcr_func_candidates.csv` from the fish registration folder or an explicit `--source-root`, refuses overwrite without `--force-recompute`, checks that ROI/anatomy inputs remain geometry-only, and checks accepted HCR final-pair schema/semantics. HCR activity table recomputation remains a later identity-specific promotion step and must replay the HCR-centric label-first candidate population rather than derive it from the ROI-centric master table.
...
- `make-figures` is a mixed render/baseline writer stage. It requires staged canonical export CSV inputs, renders the responsive identity donut and HCR anatomy coexpression summary from staged canonical CSVs, copies the remaining declared legacy figure PNG artifacts from the fish `04_plots` folder or an explicit `--figure-input-root`, writes them under `--pipeline-root/make-figures/04_plots`, and refuses to overwrite existing staged figure outputs unless `--force-recompute` is set. Replacing the remaining copied figures remains later figure-specific promotion work.
- On `L395_f11`, baseline `assign-hcr-identity --strict` now runs from a current-worktree copy on `linnaeus` with a temporary `--pipeline-root`: it writes all 7 staged identity/HCR CSVs and `compare-staged --stage-name assign-hcr-identity` passes with zero failed or warning checks.
- On `L395_f11`, dependency-aware `assign-hcr-identity --strict` now runs after staged `match-roi-to-anatomy` and `register-hcr-to-anatomy` in one temporary `--pipeline-root`: it uses the default staged dependency roots, sees 4,530 ROI/anatomy geometry rows, verifies the ROI/anatomy table is geometry-only, sees 4 staged HCR/anatomy label TIFFs and 162 accepted final-pair rows, recomputes a 137-row `anatomy_identity_lookup.csv` from staged final-pair CSVs with accepted control parity, recomputes a 4,530-row ROI identity master with accepted keyed/exact/numeric parity, and then writes all 7 staged identity/HCR CSVs.
- On `L395_f11`, `audit-hcr-activity-replay --strict` now runs from a current-worktree copy on `linnaeus` after staged five-plane `prepare-functional-reference-stacks`, `register-functional-to-anatomy`, and `register-hcr-to-anatomy` into `/tmp/codeants-hcr-replay-abzbAl/staged-L395`. The audit completed with status `warn`, zero errors, and promotion disabled. The current AntsPyx-instrumented run reports selected `ants_rigid_affine` transformlists for all five replay planes, `ants_available=True`, zero missing transform files, and anatomy XY spacing `0.5964024861653645`. It matched status rows (`162/162`) but replayed `hcr_func_candidates.csv` at `64/148`, `conf_to_func_pairs_raw.csv` at `191/254`, and `conf_to_func_pairs.csv` at `22/40`; candidate key parity reported `140` missing control keys and `56` extra replay keys. The in-manifest variant scoreboard also evaluated accepted `registration/tforms_by_plane.csv`, accepted `ncc/tforms_by_plane.csv`, selected-best-Z variants, and p4 `dx=-1,dy=+1` offset variants using shape-aware affine replay; the best current persisted-input variant still had only `64` replay keys. Historical recompute-audit artifacts on `linnaeus` show perfect parity was once possible for `selected_inplane_registration_comparison_ants_transformlist`, so the missing piece is not just transform backend choice but the exact legacy plane-ref/warped-label/functional-label construction state.
...
- `register-functional-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. It reconstructs in-memory `plane_refs` from staged functional reference TIFFs, consumes the prepared in vivo anatomy NRRD, runs NCC best-z/scale search and NCC-only in-plane comparison, writes NCC caches, comparison/recommendation CSVs, warped reference TIFFs, `plane_refs_summary.json`, and a staged `registration/tforms_by_plane.csv`. The validated run wrote to `/tmp/codeants-register-func-lki3lJ/staged-L395/register-functional-to-anatomy/` from one real `L395_f11` reference input and reported best_z `124` with `216` NCC scores for `L395_f11_plane0_mcorrected_flipX`. Full ANTs parity and downstream ROI/anatomy matching remain later work.
- `register-hcr-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. The first validated mode stages accepted small aligned HCR artifacts from `03_analysis/confocal/aligned/` into `--pipeline-root/register-hcr-to-anatomy/confocal/aligned/`: aligned label/QC TIFFs, match/review/final-pair CSVs, and warp metadata JSONs. It records but does not copy multi-GB aligned intensity NRRDs. The validated run wrote to `/tmp/codeants-register-hcr-Atb5Hl/staged-L395/...`, staged 46 small artifacts, recorded 6 aligned intensity NRRD inputs, copied 0 NRRDs, and passed checks for 4 label TIFFs, 4 match CSVs, 4 final-pair CSVs, 162 accepted final-pair rows, the 12-column HCR/anatomy pair schema, `quality=good`/`pair_type=1-1`/`within_gate=True` acceptance, and 10 warp metadata JSONs. True HCR warp recomputation remains later work.
- `match-roi-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. The first validated mode is `--source-root` control-geometry staging from the accepted registration folder; it writes `functional_roi_anatomy_matches.csv`, `functional_roi_anatomy_match_by_plane.csv`, and `functional_roi_anatomy_match_plane_meta.csv` under `--pipeline-root/match-roi-to-anatomy/registration`, refuses overwrite without `--force-recompute`, and strips identity, HCR, response, BPI, and gene columns. The validated run wrote to `/tmp/codeants-match-roi-rXBNLP/staged-L395/...`, staged 4,530 ROI geometry rows across 5 planes, found 3,724 unique anatomy matches, emitted empty stderr, and reported `mode=control_geometry`. True NCC-derived ROI/anatomy recompute parity remains later work.
...
- Existing diagnostic folders such as `assign-hcr-identity/recompute-audit/` remain baseline evidence only until HCR activity recomputation is implemented and validated; `anatomy_identity_lookup.csv` and ROI identity master recomputation are now stage-owned.
