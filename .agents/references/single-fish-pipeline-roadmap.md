...
- On `L395_f11`, generated `make-qa-report --strict` now runs from a current-worktree copy on `linnaeus` with a temporary `--pipeline-root` seeded by staged canonical exports, staged ROI/anatomy geometry matches, and a real 200 px functional/anatomy center overlay: it writes `qa_report.md` and `qa_report_summary.json` with zero errors or warnings. The summary records 8 canonical tables, 5 registration overlay planes, 5 ROI/anatomy match planes, 4,530 ROIs, 3,724 unique anatomy matches, and 806 unmatched ROIs.
