...
- `register-hcr-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. The default validated mode stages accepted small aligned HCR artifacts from `03_analysis/confocal/aligned/` into `--pipeline-root/register-hcr-to-anatomy/confocal/aligned/`: aligned label/QC TIFFs, match/review/final-pair CSVs, and warp metadata JSONs. It records but does not copy multi-GB aligned intensity NRRDs. Its manifest also reports whether the current fish has the direct-ANTs recompute prerequisites: matching metadata route, raw HCR Cellpose masks, rbest/rn HCR NRRDs, rbest-to-2p transforms, and rn-to-rbest transforms. The opt-in `--recompute-direct-ants` path now recomputes HCR label TIFFs, warp metadata, and HCR/anatomy match/review/final-pair CSVs from raw HCR masks plus current direct ANTs transforms, and overlays accepted filter stats into recomputed metadata until the notebook filter policy is promoted. On real `L395_f11`, `/tmp/codeants-hcr-direct-Cjks9D` recomputed 4 HCR label TIFFs plus 4 metadata JSONs, overlaid accepted filter stats for all 4 masks, then fed `assign-hcr-identity`, `score-activity-bpi`, and `export-canonical-tables` with zero failed checks; remaining warnings were byte-parity only. The newer direct match-table run `/tmp/codeants-hcr-match-V8VB55` produced all 4 label TIFFs, 4 metadata JSONs, and 12 match/review/final-pair CSVs, but strict final-pair key parity failed against accepted controls: sst1_1 missing 3 extra 5, pth2 missing 1 extra 3, sst1_2 missing 0 extra 1, tac3b missing 7 extra 9. Therefore direct HCR/anatomy match recomputation is now exposed and gated, but not yet accepted-equivalent.
