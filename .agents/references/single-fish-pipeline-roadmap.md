...
- Top-level `status` now includes read-only `upstream_stages` summaries for concrete upstream writer surfaces: functional references, in vivo/ex vivo anatomy preparation, HCR/ex vivo Cellpose, functional/HCR registration, and ROI/anatomy matching. These summaries inventory declared inputs/outputs and freshness without rerunning stage code. Legacy preprocessing files outside the staged pipeline root do not activate a stage by themselves; persisted manifests or staged pipeline outputs do. Active upstream staged folders can raise top-level warnings or failures when required outputs are stale or incomplete.
...
- `make-qa-report` is a generated report writer stage. It requires staged canonical export CSV inputs, writes Markdown, HTML, and JSON summary artifacts under `--pipeline-root/make-qa-report/`, includes staged output status, canonical table rows, per-plane registration/matching QA summaries from staged functional/anatomy overlay and ROI/anatomy geometry CSVs, a manual review checklist, and inline previews for existing QA/figure PNG artifacts, and refuses to overwrite existing staged report artifacts unless `--force-recompute` is set. Richer biologist-facing PDF/notebook reports remain later work.
