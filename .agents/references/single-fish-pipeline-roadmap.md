...
- `register-hcr-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. The default validated mode stages accepted small aligned HCR artifacts from `03_analysis/confocal/aligned/` into `--pipeline-root/register-hcr-to-anatomy/confocal/aligned/`: aligned label/QC TIFFs, match/review/final-pair CSVs, and warp metadata JSONs. It records but does not copy multi-GB aligned intensity NRRDs. Its manifest also reports whether the current fish has the direct-ANTs recompute prerequisites: matching metadata route, raw HCR Cellpose masks, rbest/rn HCR NRRDs, rbest-to-2p transforms, and rn-to-rbest transforms. The opt-in `--recompute-direct-ants` path now recomputes HCR label TIFFs and warp metadata from raw HCR masks plus current direct ANTs transforms, stages accepted match/review/final-pair CSVs, and overlays accepted filter stats into recomputed metadata until the notebook filter policy is promoted. On real `L395_f11`, `/tmp/codeants-hcr-direct-Cjks9D` recomputed 4 HCR label TIFFs plus 4 metadata JSONs, overlaid accepted filter stats for all 4 masks, then fed `assign-hcr-identity`, `score-activity-bpi`, and `export-canonical-tables` with zero failed checks; remaining warnings were byte-parity only. Full HCR/anatomy match/final-pair recomputation and filter-stat recomputation remain later work.
