...
All declared writer stages and comparison groups in the current roadmap are implemented. The read-only `compare-staged` command accepts declared upstream writer surfaces as well as post-processing stages. It compares declared staged CSVs by header/row-count shape, declared row keys, exact semantic cells, declared numeric cells, semantic-column/key checks for narrowed tables, non-empty generated files/directories, Cellpose-specific mask filename patterns, and figure dimensions/thumbnails where available. For upstream stages that were run with explicit output overrides, same-pipeline-root persisted manifests provide the actual output paths used by status/comparison rather than forcing the default canonical path. `freeze-legacy-baseline` and `compare-legacy-baseline` now provide a first frozen-bundle surface for the same declared comparison-stage list; upstream stages without declared legacy/control files receive nonempty staged-artifact coverage until optional stronger controls are added.
...
- Top-level `status` now includes read-only `upstream_stages` summaries for concrete upstream writer surfaces: functional references, in vivo/ex vivo anatomy preparation, HCR/ex vivo Cellpose, functional/HCR registration, and ROI/anatomy matching. These summaries inventory declared inputs/outputs and freshness without rerunning stage code. Legacy preprocessing files outside the staged pipeline root do not activate a stage by themselves; persisted manifests or staged pipeline outputs do. Upstream writer manifests record `pipeline_root`; when a persisted upstream manifest records the same value, status/compare use its actual input/output paths, including explicit `--output-path`, `--output-dir`, or `--output-root` destinations, and freshness compares current filesystem state for those manifest paths. Active upstream staged folders can raise top-level warnings or failures when required outputs are stale or incomplete.
...
- `compare-staged` is read-only and uses declared staged output specs for upstream writer surfaces plus post-processing stages. For upstream activation, legacy preprocessing files outside `pipeline_outputs` do not start a comparison by themselves; staged `pipeline_outputs` or persisted upstream manifests with the same `pipeline_root` do. Same-root manifests override the inventory/comparison output paths for explicit upstream writer destinations, so temp-root anatomy NRRDs or functional-reference directories are checked where they were actually written. On real `L765_f02`, this was validated with a temp local-root, explicit in vivo anatomy NRRD output, persisted writer manifest, `compare-staged` pass, and upstream status `pass/current`. It checks CSV shape, declared row keys, exact semantic cells, and declared numeric cells where table semantics are known, while preserving byte-identical CSV comparison as warn-only. For narrowed upstream outputs such as `match-roi-to-anatomy/functional_roi_anatomy_matches.csv`, it can compare declared geometry columns against the wider accepted ROI master instead of requiring identical headers. For generated directories such as `prepare-functional-reference-stacks/functional/raw`, nonempty means at least one real file after ignoring `.DS_Store`/AppleDouble sidecars; for Cellpose segmentation directories, the file must match `*_cp_masks.tif`. Declared numeric checks currently use `1e-5` absolute tolerance, calibrated against accepted `L395_f11` BPI control-vs-staged differences. Figure checks include dependency-free PNG dimensions plus optional Pillow thumbnail MAE/RMS warnings. It does not create/freeze baseline bundles.
...
- By default, `score-activity-bpi` consumes frozen staged ROI/anatomy geometry from `match-roi-to-anatomy`, Suite2p traces, and functional experiment logs. It does not require molecular identity; use `--geometry-input-path` only for controlled validation/bootstrap with an explicit frozen geometry table.
...
- `make-qa-report` is a generated report writer stage. It requires staged canonical export CSV inputs, writes Markdown, HTML, PDF, and JSON summary artifacts under `--pipeline-root/make-qa-report/`, includes staged output status, canonical table rows, per-plane registration/matching QA summaries from staged functional/anatomy overlay and ROI/anatomy geometry CSVs, a biologist review guide with concrete questions/evidence/actions, a manual review checklist, and inline previews for existing QA/figure PNG artifacts, and refuses to overwrite existing staged report artifacts unless `--force-recompute` is set. Notebook-style read-only review notebooks remain later work if generated reports are not sufficient for biologist review.
- `make-figures` is a render writer stage. It requires staged canonical export CSV inputs plus declared `[56i]` AUC CSV inputs, renders the 50l composite, BPI all-pairs diagnostic, per-gene 56h stimulus/HCR status summary, responsive identity donut, and HCR anatomy coexpression summary under `--pipeline-root/make-figures/04_plots`, and refuses to overwrite existing staged figure outputs unless `--force-recompute` is set.
...
- On `L395_f11`, generated `make-qa-report --strict` now runs from a current-worktree copy on `linnaeus` with a temporary `--pipeline-root` seeded by staged canonical exports, staged ROI/anatomy geometry matches, and a real 200 px functional/anatomy center overlay: it writes `qa_report.md`, `qa_report.html`, `qa_report.pdf`, and `qa_report_summary.json` with zero errors or warnings. The summary records 8 canonical tables, 5 registration overlay planes, 5 ROI/anatomy match planes, 4,530 ROIs, 3,724 unique anatomy matches, and 806 unmatched ROIs. `/tmp/codeants-review-guide-xgY9jvF7` validated the new `review_guidance` JSON plus Markdown/HTML/PDF biologist review guide on real data; PDF visual checks confirmed the guide, checklist, and overlay preview pages were readable without clipping.
- On `L395_f11`, `/tmp/codeants-review-guide-xgY9jvF7` was rerun after adding image-readability metadata and semantic summary comparison to `make-qa-report`: `make-qa-report --force-recompute` passed with zero errors/warnings, `compare-staged --stage-name make-qa-report --strict` passed with zero failed/warning checks, and `qa_report_summary.json` recorded readable dimensions plus thumbnail color counts for all six declared preview images. The Markdown/HTML/PDF manual review checklist now includes a visual-check field so missing or unreadable previews are visible without inspecting JSON; staged comparison now also validates summary JSON parseability, required sections, review status, and image metadata.
...
- On `L758_f03`, `/tmp/codeants-crossfish-1783541775` validates the upstream preprocessing writer/status/compare surface on a second fish without writing canonical fish outputs. A temp `local-root` symlinked only raw/preprocessing inputs while keeping manifests and explicit outputs under `/tmp`; `prepare-in-vivo-anatomy-stack --write-manifest` passed, `compare-staged --stage-name prepare-in-vivo-anatomy-stack` passed, `prepare-functional-reference-stacks --write-manifest` wrote 20 raw/norm reference outputs and passed, `compare-staged --stage-name prepare-functional-reference-stacks` passed, and upstream status reports both preprocessing stages `pass/current`. The run fixed status inventory for functional-reference manifests so exact writer output files are re-described instead of being replaced by a summarized directory record.
- On `L765_f03`, `/tmp/codeants-preprocess-crossfish-1783591488` validates the upstream preprocessing writer/status/compare surface on a third fish without writing canonical fish outputs. A temp `local-root` symlinked raw/preprocessing inputs, `prepare-in-vivo-anatomy-stack --write-manifest --output-path` passed with uint8 `(750, 750)` Y/X output, `prepare-functional-reference-stacks --write-manifest` wrote 20 raw/norm reference outputs from 10 planes, and stage-specific `compare-staged --strict` for both preprocessing writers exited 0. Upstream status reports both persisted manifests `current`; aggregate status is overall `fail` only because unrelated downstream controls are absent from the temp root.
- `register-functional-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. It reconstructs in-memory `plane_refs` from staged functional reference TIFFs, consumes the prepared in vivo anatomy NRRD, runs NCC best-z/scale search and in-plane comparison, writes NCC caches, comparison/recommendation CSVs, warped reference TIFFs, `plane_refs_summary.json`, and a staged `registration/tforms_by_plane.csv`. When anatomy labels and anatomy-space functional labels are available, it now also emits a 200 px center-crop functional/anatomy outline overlay QA PNG/CSV under the stage `qa/` folder; this renderer now uses NRRD-aware image IO for canonical prepared anatomy NRRDs. The CLI exposes `--ants-fixed-mask-json` and `--no-ants-fixed-mask-required`, threading the notebook `[20]` fixed-region mask contract into the package writer instead of relying on notebook globals, and now also accepts repeatable `--reference-plane-index` values for bounded representative-plane validation without manual reference copying. Selected transform rows keep the original plane index so downstream Suite2p lookup remains correct. The writer now uses the notebook `[16]` scale-search defaults (`NCC_SCALE_COARSE=(0.50, 1.50, 0.05)` plus fine/extra/ultra-fine refinements) instead of the earlier constrained `0.9-1.0` sweep and passes anatomy XY voxel spacing into the ANTs in-plane comparison. The validated NCC run wrote to `/tmp/codeants-register-func-lki3lJ/staged-L395/register-functional-to-anatomy/` from one real `L395_f11` reference input and reported best_z `124` with `216` NCC scores for `L395_f11_plane0_mcorrected_flipX`; `/tmp/codeants-review-guide-xgY9jvF7` rendered the five-plane overlay from the real anatomy NRRD with zero register-stage warnings after the NRRD IO fix. `/tmp/codeants-ants-active-real-1783346100` validated a bounded one-plane ANTs-active run with a temp full-field fixed mask JSON. `/tmp/codeants-ants-spacing-real-1783412010` validated five real `L395_f11` staged ANTs transformlists with the canonical per-plane fixed-region mask: `5/5` plane refs used `tform_src=ants_rigid_affine`, scaled reference shapes matched accepted (`531/531/532/536/536`), and ANTs `.mat` fixed parameters matched accepted exactly after voxel-spacing propagation.
- On `L758_f03`, `/tmp/codeants-planeidx-real-1783544725` validates the first-class bounded non-L395 functional-to-anatomy registration path. A two-plane `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run against the full staged reference directory passed with zero errors/warnings, discovered 10 references, selected 2 references, wrote NCC scale/best-z caches, in-plane comparison/recommendation CSVs, NCC warped references, a 2-row `registration/tforms_by_plane.csv` preserving plane indices 0/1, and `plane_refs_summary.json`. Upstream status reports `register-functional-to-anatomy pass`; `compare-staged --stage-name register-functional-to-anatomy --strict` exits 0 with zero failed checks.
...
- `match-roi-to-anatomy` has focused local contract and CLI coverage plus `linnaeus` real-data validation on `L395_f11`. The promoted recompute mode consumes staged `register-functional-to-anatomy` plane refs, Suite2p, anatomy labels, anatomy XY spacing, and notebook-equivalent functional orientation before writing `functional_roi_anatomy_matches.csv`, `functional_roi_anatomy_match_by_plane.csv`, and `functional_roi_anatomy_match_plane_meta.csv` under `--pipeline-root/match-roi-to-anatomy/registration`. It now preserves staged `ants_rigid_affine` transformlists from newly recomputed registration and uses selected accepted transformlists only when staged ANTs transformlists are absent. The validated accepted-transform fallback run wrote to `/tmp/codeants-match-recompute-promote-RrFHvB/staged-L395/...`, reported `mode=recompute_from_staged_registration`, overlaid 5 selected accepted ANTs transformlists with 0 missing transform files, wrote 4,530 geometry rows across 5 planes, found 3,724 unique anatomy matches, and passed accepted-control key, label, and unique-match parity (`4530/4530`). The corrected full staged ANTs path from `/tmp/codeants-ants-spacing-real-1783412010` reached matching with `staged_ants_transformlist_count=5`, `selected_ants_overlay_count=0`, and `selected_ants_missing_transform_files=0`; after mask, scale, and spacing alignment it has 8 boundary ROI differences (`anat_label=4522/4530`, unique matches `4523/4530`) and is promoted with bounded warnings because exact key parity passes and drift is below `0.2%`. A seed `123` deterministic rerun from `/tmp/codeants-ants-seed-real-1783412998` produced accepted parity `4516/4530` labels and `4521/4530` unique flags and still fails the drift ceiling. Control-geometry `--source-root` staging remains available for bootstrap/diagnostic use.
- On `L758_f03`, the same two-plane `/tmp/codeants-planeidx-real-1783544725/staged-L758_f03-planeidx` root now validates non-L395 ROI/anatomy geometry recompute from the first-class plane-index registration path. After adding a temp-root symlink to canonical Suite2p, `match-roi-to-anatomy --write-manifest` from the two-plane registration summary and canonical anatomy label TIFF passed with zero errors/warnings, loaded 2 Suite2p planes, wrote 687 geometry rows, found 225 unique anatomy matches, and kept geometry-only columns. Upstream status reports `match-roi-to-anatomy pass`; `compare-staged --stage-name match-roi-to-anatomy --strict` exits 0 with zero failed checks. The aggregate `status` payload is overall `fail` in this temp local root only because unrelated legacy downstream/control files are intentionally absent.
- On `L765_f03`, the preprocessing temp root `/tmp/codeants-preprocess-crossfish-1783591488/staged-L765_f03` now validates non-L395 bounded registration and ROI/anatomy matching from the first-class plane-index path. `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` passed with zero errors/warnings, discovered 10 functional references, selected 2, wrote NCC cache/comparison outputs, and preserved plane indices 0/1 in a 2-row transform table. After adding a temp-root Suite2p symlink and using the canonical structural Cellpose labels, `match-roi-to-anatomy --write-manifest` passed with zero errors/warnings and wrote 480 geometry rows, including 288 nonzero anatomy-label assignments across 215 unique nonzero labels. Stage-specific compare-staged commands report `status=warn` only because accepted/control outputs are absent for this fish while staged outputs are present/nonempty; aggregate status remains overall `fail` because unrelated downstream controls are intentionally absent.
- On `L758_f06`, `/tmp/codeants-crossfish-L758f06-1783596306/staged-L758_f06` validates another non-L395 temp-root chain through preprocessing, bounded registration, and ROI/anatomy matching. `prepare-in-vivo-anatomy-stack` and `prepare-functional-reference-stacks` passed with zero errors/warnings and preprocessing compare-staged commands passed. `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` passed with zero errors/warnings, discovered 10 functional references, selected 2, and preserved plane indices 0/1 in a 2-row transform table. `match-roi-to-anatomy --write-manifest` passed with zero errors/warnings and wrote 947 geometry rows, including 443 nonzero anatomy-label assignments across 372 unique nonzero labels. Registration/matching compare-staged commands report `status=warn` only because accepted/control outputs are absent for this fish while staged outputs are present/nonempty; aggregate status remains overall `fail` because unrelated downstream controls are intentionally absent.
- On `L758_f07`, `/tmp/codeants-crossfish-L758f07-1783597131/staged-L758_f07` validates another non-L395 temp-root chain through preprocessing, bounded registration, and ROI/anatomy matching. `prepare-in-vivo-anatomy-stack` and `prepare-functional-reference-stacks` passed with zero errors/warnings and preprocessing compare-staged commands passed. `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` passed with zero errors/warnings, discovered 10 functional references, selected 2, and preserved plane indices 0/1 in a 2-row transform table. `match-roi-to-anatomy --write-manifest` passed with zero errors/warnings and wrote 1,064 geometry rows, including 498 nonzero anatomy-label assignments across 377 unique nonzero labels. Registration/matching compare-staged commands report `status=warn` only because accepted/control outputs are absent for this fish while staged outputs are present/nonempty; aggregate status remains overall `fail` because unrelated downstream controls are intentionally absent.
- On `L765_f04`, `/tmp/codeants-crossfish-L765f04-1783597878/staged-L765_f04` validates another non-L395 temp-root chain through preprocessing, bounded registration, and ROI/anatomy matching. `prepare-in-vivo-anatomy-stack` and `prepare-functional-reference-stacks` passed with zero errors/warnings and preprocessing compare-staged commands passed. `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` passed with zero errors/warnings, discovered 10 functional references, selected 2, and preserved plane indices 0/1 in a 2-row transform table. `match-roi-to-anatomy --write-manifest` passed with zero errors/warnings and wrote 721 geometry rows, including 479 nonzero anatomy-label assignments across 406 unique nonzero labels. Registration/matching compare-staged commands report `status=warn` only because accepted/control outputs are absent for this fish while staged outputs are present/nonempty; aggregate status remains overall `fail` because unrelated downstream controls are intentionally absent.
...
- `segment-hcr-cellpose` and `segment-ex-vivo-anatomy-cellpose` have focused local manifest-contract coverage and fresh Helga GPU validation. Fresh `L765_f02` ex vivo anatomy segmentation reproduced the accepted 15,530-label mask byte-for-byte. Fresh `L395_f11` HCR segmentation ran both rbest markers under Cellpose 4.0.8; an independent `tac3b` repeat was byte-identical, while an isolated Cellpose 4.0.7 run also produced the same fresh mask. Those fresh masks differ from the unmanifested November 2025 accepted masks, and direct downstream matching retained `8/9` accepted `sst1.2` and `46/62` accepted `tac3b` anatomy identities. An NRRD-header anisotropy diagnostic produced severe under-segmentation and was not promoted. By explicit user decision, HCR segmentation parity is deferred to the planned replacement segmentation workflow rather than blocking this roadmap. Writer manifests now record Cellpose/Torch/Python/platform/CUDA/device provenance and model SHA-256; CLI Cellpose logs are captured so stdout remains valid JSON.
...
- `bpi_all_pairs.png` and `per_gene_stimulus_trace_with_hcr_status_56h.png` are package-rendered from staged canonical CSVs plus declared `[56i]` AUC plot tables. The 56h renderer is table-driven: it summarizes per-gene stimulus-window AUC by side/mode and pairs that with HCR functional-status counts, because the original single-fish trace payload is not persisted in the staged figure contract.
...
Current generated-report coverage:

- `make-qa-report` now provides a first generated biologist review surface in Markdown, HTML, PDF, and JSON.
- The report is read-only with respect to scientific outputs: it summarizes staged artifacts, review questions, evidence, and recommended actions, and it does not recompute matching, identity, response, BPI, or figures.
- Real `L395_f11` rendering in `/tmp/codeants-review-guide-xgY9jvF7` passed `make-qa-report --strict`, `compare-staged --stage-name make-qa-report`, structured JSON checks, and PDF/overlay visual inspection.
- A dedicated read-only review suite now lives under `notebooks/qc/`. It uses
  one notebook per human scientific gate rather than per executable stage:
  pipeline overview; functional-reference/drift; functional registration;
  ROI/anatomy geometry; molecular geometry; molecular identity; and
  activity/export review.
- Shared `qc_notebooks.py` infrastructure validates fish/root/manifest
  identity and permits only an explicit hash-bound, non-promoting review JSON
  under the reviewed stage's `reviews/` directory. Plotting and review-table
  logic remains package-owned in `plots.qc_functional`, `plots.qc_geometry`,
  and `plots.qc_molecular`; notebook cells remain orchestration-thin.
- These notebooks complement rather than replace `make-qa-report`: the report
  is a portable generated summary, while the notebooks support interactive
  gate-specific inspection. Neither surface promotes outputs.
