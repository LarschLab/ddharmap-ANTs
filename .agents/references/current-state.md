...
- Early anatomy preprocessing now includes `[14a]` before voxel inference `[8]`; it writes a signed-16-bit-corrected, functional-orientation-matched 8-bit anatomy TIFF with anatomy Z flipped to match the bottom-to-top confocal registration convention and Y/X resized to `750x750` in `02_reg/00_preprocessing/2p_anatomy`, also writes/backfills the canonical uncompressed registration-ready NRRD sibling `<fish_id>_anatomy_2P_GCaMP.nrrd` with spatial header metadata, rebinds `ANAT_STACK_PATH` downstream to the TIFF, and avoids chained `*_uint8_uint8.tif` outputs on rerun.
