...
- CLI wrapper: `tools/single_fish_pipeline.py` currently exposes read-only `contracts`, `audit-inputs`, `status`, `stage-status`, post-preprocessing `compare-staged`, frozen-bundle `freeze-legacy-baseline` / `compare-legacy-baseline`, read-only `audit-score-activity-bpi`, downstream writers `assign-hcr-identity`, `score-activity-bpi`, `export-canonical-tables`, `make-qa-report`, and `make-figures`, plus granular upstream writers `prepare-functional-reference-stacks`, `prepare-in-vivo-anatomy-stack`, `prepare-ex-vivo-anatomy-stack`, `segment-ex-vivo-anatomy-cellpose`, `segment-hcr-cellpose`, NCC-only `register-functional-to-anatomy`, HCR `register-hcr-to-anatomy` with opt-in `--recompute-direct-ants`, and control-geometry `match-roi-to-anatomy`; `assign-hcr-identity` accepts `--roi-anatomy-root` and `--hcr-anatomy-root` overrides for dependency validation. `--write-manifest` persists manifests for manifest-bearing commands. Later HCR/anatomy match/final-pair recomputation, HCR filter-stat recomputation, and broader comparison commands remain roadmap targets.
