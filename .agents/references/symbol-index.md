...
- `run_match_roi_to_anatomy_stage`: Writer stage for geometry-only ROI/anatomy matches. The recompute mode resolves direct-versus-reversed anatomy-label Z order against the anatomy intensity stack (or consumes an explicit mode), persists every resolved label page in `plane_refs_summary_geometry.json`, and uses those exact pages for matching. It also consumes staged transforms, Suite2p, anatomy XY spacing, and canonical functional orientation before writing the three geometry CSVs. Missing or ambiguous label-Z provenance fails closed; control-geometry staging from an explicit `--source-root` remains available for bootstrap/diagnostic use.
