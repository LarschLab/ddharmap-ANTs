...
- `run_single_fish_make_qa_report_stage`: Generate staged `qa_report.md`, `qa_report.html`, `qa_report.pdf`, and `qa_report_summary.json` under `pipeline_root/make-qa-report` from staged canonical export inputs, including canonical table rows, staged output status, per-plane registration/matching QA summaries from existing staged overlay/match CSVs, a manual review checklist, and inline image previews for existing QA/figure PNG artifacts; refuses overwrite without `force_recompute`.
