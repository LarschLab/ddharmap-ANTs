...
- `run_single_fish_make_figures_stage`: Stage the declared baseline figure artifacts under `pipeline_root/make-figures/04_plots` after verifying staged canonical export CSV inputs, rendering package-owned `compound_50j_56i_unified.png`, responsive identity donut, and HCR anatomy coexpression summary from declared table inputs, copying the remaining legacy figure artifacts, and refusing overwrite without `force_recompute`.
