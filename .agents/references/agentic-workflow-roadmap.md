...
- `make-figures` is now a package-owned mixed render/baseline writer stage. It requires staged canonical export CSV inputs plus declared `[56i]` AUC CSV inputs, renders `compound_50j_56i_unified.*`, `single_fish_50l_responsive_identity_donut.*`, and `single_fish_hcr_anatomy_coexpression_summary.*`, copies the remaining declared legacy figure PNGs under `make-figures/04_plots`, and refuses overwrite without `--force-recompute`.
...
- Stale-state reporting covers persisted `audit-inputs` inputs, persisted read-only downstream stage manifests, and warning-level freshness checks for existing downstream staged outputs. `stage-status` and top-level `status` now warn when required stage outputs are older than required declared inputs; deeper cross-stage dependency freshness for all upstream roots remains a later expansion.
...
3. With ROI identity recompute, HCR activity replay/promotion, direct-HCR label warp recompute, ROI/anatomy recompute, the 50l composite figure render, and downstream staged output freshness warnings promoted, the next `linnaeus`-validated slice should either replace one of the two remaining copied figure artifacts with a package renderer, deepen cross-stage freshness for upstream roots, or broaden biologist-facing QA around the validated registration/matching surfaces.
