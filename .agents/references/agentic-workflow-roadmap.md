...
3. With ROI identity recompute, HCR activity replay/promotion, direct-HCR label warp recompute, ROI/anatomy recompute, and the 50l composite figure render promoted, the next `linnaeus`-validated slice should either replace one of the two remaining copied figure artifacts with a package renderer, harden dependency/freshness reporting for validated upstream roots, or broaden biologist-facing QA around the validated registration/matching surfaces.
