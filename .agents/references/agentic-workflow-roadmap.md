...
- `make-qa-report` is now a package-owned generated report writer stage with focused local and `L395_f11` real-data validation. It requires staged canonical export CSV inputs, writes `qa_report.md` and `qa_report_summary.json` under `make-qa-report/`, includes registration/matching QA tables from staged functional/anatomy overlay and ROI/anatomy geometry CSVs, a manual review checklist, and inline previews for existing QA/figure PNG artifacts, and refuses overwrite without `--force-recompute`.
...
3. With ROI identity recompute, HCR activity replay/promotion, direct-HCR label warp recompute, ROI/anatomy recompute, the 50l composite, BPI all-pairs, per-gene 56h stimulus/HCR status figure renders, registration/matching QA report summaries, downstream/dependency freshness warnings, and read-only upstream freshness summaries promoted, the next `linnaeus`-validated slice should validate the richer report on real staged registration/matching outputs and then continue toward HTML/PDF/notebook-style review surfaces.
...
```text
rsync current worktree to /tmp/codeants-qa-regmatch-1783319839/codeANTs on linnaeus, seed /tmp/codeants-qa-regmatch-1783319839/staged-L395 with real staged canonical/match outputs from /tmp/codeants-match-recompute-promote-RrFHvB/staged-L395, render the 200 px functional/anatomy center overlay from real L395_f11 anatomy/label/functional-mask images, then run:

PYTHONPATH=src /Users/ddharmap/miniforge3/bin/python tools/single_fish_pipeline.py make-qa-report --fish-id L395_f11 --local-root /Volumes/dataDrive/dataProcessing/2p_processing --strict --pipeline-root /tmp/codeants-qa-regmatch-1783319839/staged-L395 --force-recompute
status: pass
errors: []
warnings: []
