...
- On `L395_f11`, `/tmp/codeants-review-guide-xgY9jvF7` validated the generated biologist review guide on real staged registration/matching outputs. `register-functional-to-anatomy` rendered a five-plane functional/anatomy center overlay from the canonical in vivo anatomy NRRD after the QA renderer was fixed to use NRRD-aware image IO; `make-qa-report --strict` and `compare-staged --stage-name make-qa-report` passed with zero warnings/errors. Visual verification confirmed the overlay PNG was nonblank/populated and the PDF review-guide, checklist, and overlay-preview pages were readable without clipping.
- On `L395_f11`, the same `/tmp/codeants-review-guide-xgY9jvF7` root now validates stronger visual-review report metadata. `make-qa-report --force-recompute` records image byte size, readability, dimensions, and thumbnail color count for declared image artifacts, surfaces a visual-check column in the Markdown/HTML/PDF checklist, and distinguishes missing images from unreadable image files in review guidance. `compare-staged --stage-name make-qa-report` now semantically checks `qa_report_summary.json` for parseability, required sections, valid review status, and image metadata/readability. The real rerun passed with zero errors/warnings; the real semantic compare passed with zero failed/warning checks; all six declared preview images were readable with dimensions recorded in `qa_report_summary.json` (functional/anatomy overlay plus five staged figure PNGs).
...
- `register-functional-to-anatomy` has focused local contract coverage plus `linnaeus` real-data validation on `L395_f11` as the first functional-to-anatomy registration writer. It reconstructs lightweight `plane_refs` from staged functional reference TIFFs, runs the package-owned NCC best-z/scale search and in-plane comparison, writes NCC caches, warped-reference artifacts, `plane_refs_summary.json`, and a staged `tforms_by_plane.csv`. The CLI now exposes `--ants-fixed-mask-json` / `--no-ants-fixed-mask-required` so the package writer can run the ANTs in-plane backend without notebook globals, `--ants-deterministic-seed` for opt-in deterministic ANTs diagnostics, and repeatable `--reference-plane-index` for bounded representative-plane validation while preserving original plane indices in outputs. The package writer now uses the notebook `[16]` scale-search defaults (`0.50-1.50` coarse plus fine refinements) and passes anatomy XY voxel spacing into ANTs in-plane registration. `/tmp/codeants-ants-spacing-real-1783412010` validated five real `L395_f11` staged ANTs transformlists with canonical per-plane fixed masks, accepted-like scaled reference shapes (`531/531/532/536/536`), and accepted-matching ANTs fixed parameters. `/tmp/codeants-ants-seed-real-1783412998` showed seed `123` is not a parity fix: matching from seeded transforms reached `4516/4530` label parity and `4521/4530` unique-match parity, worse than the unseeded 8-row residual.
...
- `match-roi-to-anatomy` has focused local contract coverage plus `linnaeus` real-data validation on `L395_f11` as the first ROI/anatomy geometry writer. The promoted recompute mode consumes staged `register-functional-to-anatomy` plane refs, Suite2p, anatomy labels, anatomy XY spacing, and notebook-equivalent functional orientation before writing geometry-only ROI/anatomy match tables under `match-roi-to-anatomy/registration/`; it now preserves staged `ants_rigid_affine` transformlists from newly recomputed registration and uses selected accepted transformlists only as a fallback. Accepted-control key, anatomy-label, selected-label, and unique-match parity pass exactly for the accepted-transform fallback path. The full five-plane staged ANTs path reaches matching with `staged_ants_transformlist_count=5`, `selected_ants_overlay_count=0`, and no missing transform files; after mask, scale, and spacing alignment, accepted geometry parity is narrowed to 8 differing boundary ROIs (`anat_label=4522/4530`, unique matches `4523/4530`). Candidate reconstruction showed these are transform-induced boundary/competition differences, not matching tie-break artifacts. Regenerated ANTs geometry now has a documented promotion gate: exact key parity remains required, label/unique drift up to `0.2%` is warning-only, and larger drift fails. On `/tmp/codeants-ants-spacing-real-1783412010`, `match-roi-to-anatomy --strict` passes with the two bounded-drift warnings, and `compare-staged --stage-name match-roi-to-anatomy --strict` reports `status=warn` with `23/28` allowed exact-cell differences. The seed `123` negative control still fails at `37/28`.
- `segment-hcr-cellpose` and `segment-ex-vivo-anatomy-cellpose` have focused local manifest-contract coverage plus positive Helga GPU execution. Under `C:\Users\zebrafish\codeants_jobs\roadmap_validation_20260712`, current code and Cellpose 4.0.8 freshly segmented both `L395_f11` rbest marker stacks; a second independent `tac3b` run was byte-identical to the first fresh run, proving deterministic current-runtime execution. The fresh masks did not reproduce the unmanifested November 2025 accepted masks, and downstream direct-ANTs matching retained only `8/9` accepted `sst1.2` anatomy identities and `46/62` accepted `tac3b` identities. Cellpose 4.0.7 reproduced the 4.0.8 fresh mask exactly, while an NRRD-header anisotropy diagnostic materially worsened segmentation and was not promoted. HCR segmentation parity is therefore explicitly deferred to the planned replacement segmentation workflow rather than treated as a staged-pipeline promotion gate. Cellpose writer manifests now record Python/Cellpose/Torch/platform/CUDA/device provenance plus model SHA-256, and the CLI captures Cellpose console output so stdout remains clean JSON. Fresh `L765_f02` ex vivo anatomy segmentation on Helga produced 15,530 labels and was byte-identical to the accepted NAS mask (`SHA-256 74a81490661374258f723c2c300530628d7b498167098b8a61de03fe65c4e97c`).
- The granular Cellpose CLI now supports `--no-gpu` for CPU validation fallback while preserving GPU as the default.
- `compare-staged`, `freeze-legacy-baseline`, and `compare-legacy-baseline` now accept declared upstream writer surfaces as comparison-stage names as well as post-processing stages. Upstream comparisons activate only from staged `pipeline_outputs` or same-pipeline-root persisted upstream manifests, so legacy preprocessing files and unrelated persisted manifests do not accidentally turn into active staged comparisons. Same-pipeline-root manifests now also provide the comparison/status output paths for upstream stages that were run with explicit `--output-path`, `--output-dir`, or `--output-root` overrides. Focused local coverage verifies read-only `prepare-functional-reference-stacks`, `prepare-in-vivo-anatomy-stack`, `register-functional-to-anatomy`, and `match-roi-to-anatomy` upstream comparisons, true nonempty directory checks, Cellpose-specific `*_cp_masks.tif` directory checks, warning-level partial functional-registration parity, strict ROI/anatomy geometry failures, explicit manifest-output path handling, and unrelated-manifest non-activation.
- On `L395_f11`, `/tmp/codeants-upstream-compare-real-1783329439` validated upstream staged comparison against `/tmp/codeants-review-guide-xgY9jvF7/staged-L395`: aggregate `compare-staged --strict` exited 0 with `status=warn`, zero failed checks, warning-only `register-functional-to-anatomy` partial parity, `match-roi-to-anatomy` pass, `make-qa-report` pass, and known figure-dimension warnings for two generated figure outputs.
- On `L395_f11`, the same synced worktree validated `compare-staged --stage-name prepare-functional-reference-stacks --strict` against `/tmp/codeants-hcr-replay-abzbAl/staged-L395`; the staged functional-reference directory passed the tightened nonempty artifact check with 10 real TIFF files.
- On `L765_f02`, `/tmp/codeants-manifest-output-real-1783345403` validated the manifest-aware explicit-output path contract on real data without touching the canonical fish folder. A temp `local-root` symlinked raw/preprocessing inputs, `prepare-in-vivo-anatomy-stack --write-manifest --pipeline-root /tmp/codeants-manifest-output-real-1783345403/staged-L765 --output-path /tmp/codeants-manifest-output-real-1783345403/explicit-in-vivo/L765_f02_anatomy_2P_GCaMP.nrrd` passed, `compare-staged --stage-name prepare-in-vivo-anatomy-stack --strict` passed against the explicit NRRD/JSON outputs, and upstream status reported `pass/current` for the persisted manifest.
- On `L758_f03`, `/tmp/codeants-crossfish-1783541775` adds cross-fish upstream preprocessing evidence without writing canonical fish outputs. A temp `local-root` symlinked raw/preprocessing inputs while keeping manifests local; `prepare-in-vivo-anatomy-stack --write-manifest` and `prepare-functional-reference-stacks --write-manifest` both passed, their `compare-staged` checks passed, and upstream status now reports both stages `pass/current`. This run also fixed a status-manifest mismatch where functional-reference writer manifests listed exact raw/norm TIFF outputs but status summarized the stage as one directory.
- On `L765_f03`, `/tmp/codeants-preprocess-crossfish-1783591488` adds broader cross-fish preprocessing evidence. A temp `local-root` symlinked raw/preprocessing inputs, `prepare-in-vivo-anatomy-stack --write-manifest --output-path /tmp/codeants-preprocess-crossfish-1783591488/explicit-L765_f03/L765_f03_anatomy_2P_GCaMP.nrrd` passed with uint8 `(750, 750)` Y/X output, and `prepare-functional-reference-stacks --write-manifest` wrote 10 raw and 10 normalized functional references. Stage-specific `compare-staged --strict` for both preprocessing writers exited 0, and upstream status reports both persisted manifests `current`; aggregate status is still overall `fail` because unrelated downstream controls are absent in the temp root.
- On `L765_f03`, the same `/tmp/codeants-preprocess-crossfish-1783591488` temp root now extends cross-fish evidence through bounded registration and ROI/anatomy matching. A two-plane `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run passed with zero errors/warnings, 10 references discovered, 2 selected, NCC caches and in-plane comparison outputs, and a 2-row transform table preserving plane indices 0/1. `match-roi-to-anatomy --write-manifest` then passed with zero errors/warnings against the structural Cellpose labels and Suite2p planes, writing 480 geometry rows with 288 nonzero anatomy-label assignments across 215 unique nonzero labels. Stage-specific compare-staged commands are `status=warn` only because accepted/control outputs are missing for this upstream fish while staged outputs are present/nonempty; aggregate status remains overall `fail` because unrelated downstream controls are absent.
- On `L758_f06`, `/tmp/codeants-crossfish-L758f06-1783596306` adds another independent cross-fish temp-root validation without writing canonical fish outputs. After correcting temp symlinks to mounted raw/preprocessed/Suite2p inputs, `prepare-in-vivo-anatomy-stack --write-manifest --output-path` and `prepare-functional-reference-stacks --write-manifest` passed with zero errors/warnings; preprocessing compare-staged commands passed. The bounded `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run passed with 10 references discovered, 2 selected, and a 2-row transform table preserving plane indices 0/1. `match-roi-to-anatomy --write-manifest` passed with 947 geometry rows, 443 nonzero anatomy-label assignments, and 372 unique nonzero labels. Registration/matching compare-staged commands are warning-only missing-control checks; aggregate status remains overall `fail` because unrelated downstream controls are absent.
- On `L758_f07`, `/tmp/codeants-crossfish-L758f07-1783597131` adds another independent cross-fish temp-root validation without writing canonical fish outputs. `prepare-in-vivo-anatomy-stack --write-manifest --output-path` and `prepare-functional-reference-stacks --write-manifest` passed with zero errors/warnings; preprocessing compare-staged commands passed. The bounded `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run passed with 10 references discovered, 2 selected, and a 2-row transform table preserving plane indices 0/1. `match-roi-to-anatomy --write-manifest` passed with 1,064 geometry rows, 498 nonzero anatomy-label assignments, and 377 unique nonzero labels. Registration/matching compare-staged commands are warning-only missing-control checks; aggregate status remains overall `fail` because unrelated downstream controls are absent.
- On `L765_f04`, `/tmp/codeants-crossfish-L765f04-1783597878` adds another independent cross-fish temp-root validation without writing canonical fish outputs. `prepare-in-vivo-anatomy-stack --write-manifest --output-path` and `prepare-functional-reference-stacks --write-manifest` passed with zero errors/warnings; preprocessing compare-staged commands passed. The bounded `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run passed with 10 references discovered, 2 selected, and a 2-row transform table preserving plane indices 0/1. `match-roi-to-anatomy --write-manifest` passed with 721 geometry rows, 479 nonzero anatomy-label assignments, and 406 unique nonzero labels. Registration/matching compare-staged commands are warning-only missing-control checks; aggregate status remains overall `fail` because unrelated downstream controls are absent.
- On `L758_f03`, `/tmp/codeants-planeidx-real-1783544725` validates the first-class bounded multi-plane functional-to-anatomy registration path. A two-plane `register-functional-to-anatomy --reference-plane-index 0 --reference-plane-index 1 --write-manifest --skip-visual-qa` run passed with 10 references discovered, 2 selected, NCC best-z cache, in-plane comparison CSV, a 2-row transform table preserving plane indices 0/1, and plane refs summary. Upstream status reports `register-functional-to-anatomy pass`; `compare-staged --stage-name register-functional-to-anatomy --strict` exited 0 with zero failed checks.
- On `L758_f03`, the same first-class plane-index root reaches ROI/anatomy geometry matching. After adding the temp-root Suite2p symlink, `match-roi-to-anatomy --write-manifest` passed with 2 Suite2p planes loaded, 687 ROI/anatomy geometry rows, 225 unique anatomy matches, geometry-only columns, and no warnings. Upstream status reports `match-roi-to-anatomy pass`; `compare-staged --stage-name match-roi-to-anatomy --strict` exited 0 with zero failed checks. The aggregate `status` command is still overall `fail` in that temp local root because unrelated legacy downstream/control files are intentionally absent.
- `L395_f11` existing staged outputs are the first baseline/control for staged parity checks.
- Generic preprocessing is no longer entirely out of scope: concrete granular writers now exist for in vivo anatomy preparation, ex vivo anatomy preparation, and Cellpose segmentation surfaces. The generic roadmap contract still assumes post-preprocessing inputs for `audit-inputs`/`status`; staged comparison support now includes upstream writer outputs where declared.
...
- The current post-preprocessing staged folders all have writer surfaces. `assign-hcr-identity` now recomputes the anatomy identity lookup, ROI identity master, HCR activity/status/candidate CSV family, and HCR activity status summary from staged dependencies; `score-activity-bpi` recomputes activity/BPI; `export-canonical-tables` assembles staged canonical CSVs; `make-qa-report` generates Markdown/HTML/PDF/JSON review artifacts with registration/matching QA summaries, a biologist review guide, a manual review checklist, and image readability/dimension metadata; and `make-figures` renders all declared final figure PNGs from staged canonical CSVs plus declared `[56i]` AUC table inputs. Upstream preprocessing/registration/matching writers now include focused functional-reference, in vivo/ex vivo anatomy, fresh positive Cellpose execution, full five-plane ANTs-active functional-to-anatomy backend validation with bounded-drift ROI geometry promotion on `L395_f11`, direct-HCR-to-anatomy recompute, ROI/anatomy recompute slices, and bounded non-baseline registration/matching evidence on `L758_f03`, `L765_f03`, `L758_f06`, `L758_f07`, and `L765_f04`. Dedicated review notebooks and additional cross-fish promotion evidence remain optional extensions; HCR segmentation parity is deferred to the planned replacement segmentation workflow by explicit user decision.
- `compare-staged` compares declared existing staged outputs with CSV shape checks, semantic-column/key/exact/numeric checks for narrowed tables, true nonempty file/directory checks, Cellpose mask-pattern checks, and PNG dimension checks where relevant. `freeze-legacy-baseline` and `compare-legacy-baseline` now accept the same declared comparison-stage names. Upstream preprocessing/registration comparison coverage has real `L395_f11` evidence for functional references, registration, matching, fresh HCR Cellpose execution, and byte-identical fresh ex vivo anatomy segmentation, plus `L758_f03`, `L765_f03`, `L758_f06`, `L758_f07`, and `L765_f04` cross-fish evidence for in-vivo anatomy / functional-reference preprocessing and two-plane functional registration plus ROI/anatomy geometry. When an upstream fish lacks accepted control files, `compare-staged` reports warning-only missing-control checks if staged outputs are present. Additional cross-fish promotion evidence with stronger downstream controls is optional rather than required for the current roadmap.
- `audit-score-activity-bpi` remains read-only; `score-activity-bpi` promotes recomputed activity/BPI tables into staged CSV outputs. `export-canonical-tables` promotes staged score/HCR registration CSVs into the canonical export bundle and consumes the promoted staged `assign-hcr-identity` outputs by default; explicit HCR/control roots remain available only for controlled bootstrap and validation.
...
- Stale-state reporting covers persisted `audit-inputs` inputs, persisted read-only downstream stage manifests, warning-level freshness checks for existing downstream staged outputs, dependency freshness checks for existing staged roots that were previously optional/bootstrap inputs, and read-only upstream-stage inventory/freshness summaries. `stage-status` and top-level `status` now warn when active required stage outputs are older than required declared inputs or existing upstream staged dependencies.
...
- The older `/Volumes/dataDrive/dataProcessing/2p_processing/L765_f02` mirror on `linnaeus` lacks ex vivo/rbest inputs, but the complete fish folder is accessible at `/Volumes/jlarsch/default/D2c/07_Data/Danin/Microscopy/L765_f02`; those NAS inputs supplied the completed fresh ex vivo validation.
...
The declared hybrid single-fish roadmap is complete: it is implemented and validated through preprocessing writers, functional/HCR registration, ROI/anatomy matching, identity, activity/BPI, canonical exports, figures, and generated QA reports. There is no required next implementation slice. Dedicated review notebooks and additional cross-fish promotion controls are optional extensions, not unfinished requirements. Fresh HCR segmentation parity is intentionally deferred to the planned replacement segmentation workflow; accepted HCR masks remain authoritative until that replacement is defined and validated.
...
```text
rsync current worktree to /tmp/codeants-qa-html-1783320365/codeANTs on linnaeus, seed /tmp/codeants-qa-html-1783320365/staged-L395 with real staged canonical/match/overlay outputs from /tmp/codeants-qa-regmatch-1783319839/staged-L395, then run:

PYTHONPATH=src /Users/ddharmap/miniforge3/bin/python tools/single_fish_pipeline.py make-qa-report --fish-id L395_f11 --local-root /Volumes/dataDrive/dataProcessing/2p_processing --strict --pipeline-root /tmp/codeants-qa-html-1783320365/staged-L395 --force-recompute
status: pass
errors: []
warnings: []
outputs: qa_report.md, qa_report.html, qa_report.pdf, qa_report_summary.json
qa_report_summary.json records 8 canonical tables, 5 registration overlay planes, 5 ROI/anatomy match planes, 4,530 ROIs, 3,724 unique anatomy matches, and 806 unmatched ROIs.
qa_report.html is nonempty and includes Registration And Matching QA plus Visual Artifact Preview. qa_report.pdf is nonempty and renders with readable summary/table/preview pages.

PYTHONPATH=src /Users/ddharmap/miniforge3/bin/python tools/single_fish_pipeline.py compare-staged --fish-id L395_f11 --local-root /Volumes/dataDrive/dataProcessing/2p_processing --strict --pipeline-root /tmp/codeants-qa-html-1783320365/staged-L395 --stage-name make-qa-report
status: pass
failed checks: 0
warning checks: 0
```
