...
- `assign-hcr-identity` is now a package-owned hybrid writer stage with focused local and `L395_f11` real-data validation. It requires staged ROI/anatomy geometry and staged HCR/anatomy artifacts, recomputes `anatomy_identity_lookup.csv` from staged HCR final-pair CSVs using the accepted gene order, recomputes `functional_roi_activity_identity.csv` by attaching that lookup to staged ROI/anatomy geometry while carrying through trace-quality/response fields, stages the remaining accepted HCR registration CSVs under `assign-hcr-identity/registration`, refuses overwrite without `--force-recompute`, and checks geometry-before-identity plus accepted HCR final-pair semantics. HCR activity table recomputation remains a later promotion step.
- The response-aware HCR activity export finalizer from notebook `[50]` is now package-owned as `matching.finalize_hcr_activity_export_tables`, with local tests and an `L395_f11` real-data parity probe. It is not yet wired into `assign-hcr-identity`; full HCR activity recomputation still needs a validated staged replay/source for the HCR-centric label-first candidate population and package ownership for any remaining `[50e]` summary semantics in the active worktree.
...
- `make-qa-report` is now a package-owned generated report writer stage with focused local and `L395_f11` real-data validation. It requires staged canonical export CSV inputs, writes `qa_report.md` and `qa_report_summary.json` under `make-qa-report/`, and refuses overwrite without `--force-recompute`.
- `make-figures` is now a package-owned mixed render/baseline writer stage. It requires staged canonical export CSV inputs, renders `single_fish_50l_responsive_identity_donut.*` and `single_fish_hcr_anatomy_coexpression_summary.*` from staged canonical CSVs, copies the remaining declared legacy figure PNGs under `make-figures/04_plots`, and refuses overwrite without `--force-recompute`.
...
- `register-hcr-to-anatomy` has focused local contract coverage plus `linnaeus` real-data validation on `L395_f11` as the first HCR-to-anatomy registration writer. It stages accepted small aligned HCR artifacts from `03_analysis/confocal/aligned/` into `register-hcr-to-anatomy/confocal/aligned/`, validates label/match/final-pair/warp-metadata presence plus final-pair schema and accepted-pair semantics, records large aligned intensity NRRDs as inputs, does not copy multi-GB NRRDs into staged roots, and reports direct-ANTs recompute readiness from current raw HCR masks, rbest/rn HCR NRRDs, matching metadata, and transform files. The opt-in `--recompute-direct-ants` path now recomputes HCR label TIFFs, warp metadata, and HCR/anatomy match/review/final-pair CSVs from raw masks plus current ANTs transforms, while still using accepted filter stats until filter-stat recomputation is promoted. On `L395_f11`, the recomputed match tables are produced but final-pair key parity currently fails, so promotion remains blocked at the match-policy/filtering comparison.
- `match-roi-to-anatomy` has focused local contract coverage plus `linnaeus` real-data validation on `L395_f11` as the first ROI/anatomy geometry writer. The validated control-geometry mode stages accepted ROI/anatomy geometry columns from the legacy registration root into `match-roi-to-anatomy/registration/`, writes by-plane/metadata summaries, refuses overwrite without `--force-recompute`, and excludes identity, HCR, response, BPI, and gene columns.
...
- The current post-preprocessing staged folders all have writer surfaces. `assign-hcr-identity` now recomputes the anatomy identity lookup and ROI identity master from staged geometry artifacts but still stages the HCR activity tables from accepted registration outputs; the HCR activity finalizer and read-only label-first replay audit are package-owned, but replay parity is not yet sufficient for promotion. On `L395_f11`, the replay audit now verifies AntsPyx-backed selected `ants_rigid_affine` transformlists are available for all five planes, but still matches only `hcr_activity_status.csv` row count (`162`) and replays `64/148` candidate rows, `191/254` raw rows, and `22/40` responsive pair rows, with `140` missing and `56` extra candidate keys. The new in-manifest variant scoreboard also tests accepted `registration/tforms_by_plane.csv`, accepted `ncc/tforms_by_plane.csv`, selected-best-Z variants, and p4 `dx=-1,dy=+1` offset variants; all current persisted-input variants remain far from historical recompute-audit parity. This confirms the old "not using AntsPyx" concern is instrumented, while the remaining gap is an exact legacy plane-ref/warped-label/functional-label construction mismatch. `make-figures` renders two package-owned final figures and still copies three declared legacy figure PNGs; `score-activity-bpi` recomputes activity/BPI; `export-canonical-tables` assembles staged canonical CSVs; `make-qa-report` generates a compact Markdown/JSON review report. Upstream preprocessing/registration/matching writers now include focused functional-reference, in vivo/ex vivo anatomy, Cellpose, NCC functional-to-anatomy, HCR-to-anatomy aligned-artifact staging, and ROI/anatomy control-geometry slices. True HCR warp recomputation, HCR activity table promotion, richer biologist-facing reports, and the remaining figure-render replacements remain missing.
...
Continue after the first upstream preprocessing writer coverage:
...
3. With ROI identity recompute, the HCR activity finalizer, and the read-only HCR activity replay audit now available, the next `linnaeus`-validated slice should either close the replay parity gap for the HCR-centric label-first candidate population or harden dependency/freshness reporting for the validated upstream roots.
