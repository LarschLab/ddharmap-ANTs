...
3. With ROI identity recompute, HCR activity replay/promotion, direct-HCR label warp recompute, and ROI/anatomy recompute validated on `L395_f11`, the next `linnaeus`-validated slice should either replace one remaining copied figure artifact with a package renderer, harden dependency/freshness reporting for validated upstream roots, or broaden biologist-facing QA around the validated registration/matching surfaces.
