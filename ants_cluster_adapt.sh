...
#hallo
    ...
    "align_21dpf")
        ...
        #REF="/raven/u/${USER}/data/fixed/T_hcn__it1.nrrd_template0_8bit.nrrd"
        REF="/raven/u/${USER}/data/fixed/T_average_brain_hcn1_.nrrd_template0.nrrd"
        ...
        # Previous parameters used SyN[0.25,6,0.1] - 2023.12.22
        ANTSCALL="${ANTSBIN}/antsRegistration \
        -d 3 --float 1 --verbose 1 \
        -o [${TEMPLATE:: -${#FILETYPE}},${TEMPLATE:: -${#FILETYPE}}_aligned.tif] \
        --interpolation WelchWindowedSinc \
    ...
    "align_21dpf_HCR_to_2P")
        ...
        #REF="/raven/u/${USER}/data/fixed/20230316_F2_Zstack_00001_00001preProcessed_adjust_b.nrrd"
        #REF="/raven/u/${USER}/data/fixed/T_hcn__it1.nrrd_template0_8bit.nrrd"
        REF="/raven/u/${USER}/data/fixed/21dpf_AVG_H2BGCaMP6s_gamma_flipped.nrrd"
    ...
    "align_21dpf_param_loop")
        ...
        echo "${TEMPLATE:: -${#FILETYPE}},${TEMPLATE:: -${#FILETYPE}}_${value}_aligned.tif"
        # originally SyN [0.05, 6, 0.2], worked in half the cases
        # Previous parameters used SyN[0.25,6,0.1] - 2023.12.22
        # Tried SyN[0.25,6,0.2], but that supposedly makes the image stiffer? Anatonmy of an Ants call doc - 2023.12.28

        ANTSCALL="${ANTSBIN}/antsRegistration \
        -d 3 --float 1 --verbose 1 \
        -o [${TEMPLATE:: -${#FILETYPE}},${TEMPLATE:: -${#FILETYPE}}_gradientstep_${value}_aligned.tif] \
        --interpolation WelchWindowedSinc \
        --winsorize-image-intensities [0, 100] \
        --use-histogram-matching 1 \
        -r [${REF},${TEMPLATE},1] \
        ...
        -r ${REF} \
        -i ${TEMPLATE} \
        -o ${TEMPLATE%.*}_${value}_aligned_ref.tif \
    ...
    "align_juv_hcr_noc2_nomask")
        for TEMPLATE in /raven/u/${USER}/reg/moving_hcr/${PREFIX}; do

        REF="/raven/u/${USER}/reg/fixed_hcr/hcn1_20230426___het_invandbout_2_TileScan1_A_Merged_masked.nrrd"
        ...
        ANTSCALL="${ANTSBIN}/antsRegistration \
        -d 3 --float 1 --verbose 1 \
        -o [${TEMPLATE:: -${#FILETYPE}}_m_,${TEMPLATE:: -${#FILETYPE}}_aligned.tiff] \
        --interpolation WelchWindowedSinc \
        --winsorize-image-intensities [0.01,0.99] \
        --use-histogram-matching 1 \
        -r [${TEMPLATE},${REF},1] \
    ...
    "apply-forward") # TODO: write sbatch command
        REF="/raven/u/bluk/data/fixed/T_hcn_template0_8bit.nrrd"
        for TEMPLATE in $(find "/raven/u/${USER}/reg/moving/" -name "*.nrrd"); do
        "${ANTSBIN}/antsApplyTransforms" -d 3 --verbose 1 \
        -r "${TEMPLATE}" \
        -i "${REF}" \
    ...
    "generate_average_brain")
      ANTSCALL="${ANTSBIN}/antsMultivariateTemplateConstruction2.sh \
      -d 3 \
      -o ${OUTPATH}T_average_brain_${PREFIX}_ \
      -i 6 \
      -q 100x100x70x20 \
      -f 8x4x2x1 \
      -g 0.2 \
      ...
      ${BRAINSPATH}
      "
      echo "Pulling files from following path:"
      echo "${BRAINSPATH}"
