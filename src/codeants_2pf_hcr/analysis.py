...
def render_suite2p_stimulus_locked_trace_panels(
    *,
    ...
    for stim_type in stim_types:
        ...
        trial_count = pd.to_numeric(sub.get("n_valid_trials", pd.Series(dtype=float)), errors="coerce").dropna()
        trial_text = f"; trials/session={int(trial_count.min())}" if not trial_count.empty else ""
        blocks = sorted({block.strip() for value in sub.get("blocks_used", pd.Series(dtype=str)).dropna() for block in str(value).split(",") if block.strip()})
        block_text = f"; blocks={','.join(blocks)}" if blocks else ""
        ax.set_title(
            f"{stim_type} (neurons n={sub[['plane_idx', 'func_label']].drop_duplicates().shape[0]}{trial_text}{block_text})",
            fontsize=10,
        )
    ...
    fig.suptitle(f"{title}\nStimulus-present trials only; bold traces are session population means", y=0.995)
