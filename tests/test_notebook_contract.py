...
MULTIFISH_NOTEBOOK_PATH = REPO_ROOT / "notebooks" / "multiFish.ipynb"
...
def test_multifish_notebook_contract_has_zero_violations() -> None:
    result = check_notebook_contract(MULTIFISH_NOTEBOOK_PATH)
    assert result["notebook_path"].endswith("notebooks/multiFish.ipynb")
    assert result["profile"] == "cohort"
    assert result["n_top_level_defs"] == 0
    assert result["n_figure_violations"] == 0
    assert result["n_required_cell_violations"] == 0
